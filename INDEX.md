# Komentář k zákonu č. 159/2006 Sb., o střetu zájmů (ZSZS)

## Struktura
- [00 — Úvod](00-uvod.md)
- [01 — Celý zákon](01-cely-zakon.md)

## Klíčové instituty
- **Veřejný funkcionář** (§ 2)
- **Oznámení** — o činnostech, majetku, příjmech, závazcích
- **CRO** — Centrální registr oznámení (MSp, online dostupný)
- **Lex Babiš** (novela 14/2017):
  - § 4a — zákaz vlastnictví médií
  - § 4b — zákaz veřejných zakázek
  - § 4c — zákaz dotací
- **Pl.ÚS 13/17** — ÚS zamítavý nález (potvrdil ústavnost)
- **Přestupky** — § 23 (až 250 tis. Kč)

## Vazby
[Ústava](/home/vojtech/Komentar-Ustava-Listina-1-2-1993/), [ZZVZ 134/2016](/home/vojtech/Komentar-ZZVZ-134-2016/), [ZoO 128/2000](/home/vojtech/Komentar-ZoO-128-2000/), [ZSS 234/2014](/home/vojtech/Komentar-ZSS-234-2014/), [ZUUSC 312/2002](/home/vojtech/Komentar-ZUUSC-312-2002/).

## Právní modality

- [PRAVNI-MODALITY.md](PRAVNI-MODALITY.md) — výklad příkazů, zákazů, dovolení, zmocnění, oprávnění a povinností, domněnek a fikcí použitých v předpisu.

## Zásady z místní judikatury

- [ZASADY.md](ZASADY.md) — právní zásady a judikatorní teze extrahované výhradně z místní složky `judikatura/`, bez mazání existujících citací.

<!-- LOCAL-VYKLADOVE-OTAZKY-LINK -->
## Výkladové otázky a odpovědi

- [VYKLADOVE-OTAZKY.md](VYKLADOVE-OTAZKY.md) — předpisově konkrétní kontrolní otázky, odpovědi, místní opora a výslovně označené mezery komentáře.
